...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dx1, dw1, db1 = affine_relu_backward(dout1, cache1)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    ############################################################################
    matrix_size_list = [(input_dim, hidden_dims[0])]
    matrix_size_list.extend([(hidden_dims[i], hidden_dims[i+1]) for i in range(len(hidden_dims)-1)])
    matrix_size_list.append((hidden_dims[-1], num_classes))

    for i, size in enumerate(matrix_size_list):
      self.params['W'+str(i+1)] = weight_scale * np.random.randn(size[0], size[1])
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    loss, dscores = softmax_loss(scores, y);
    loss += .5 * self.reg * sum([np.sum(self.params['W' + str(i+1)]**2) for i in range(self.num_layers)])
  
    dout = dscores
    for i in range(self.num_layers):
      idx = self.num_layers - i
      dout, dw, db = affine_backward(dout, affine_cache_list[idx - 1]);
      ...
      if self.use_dropout:
        dout = dropout_backward(dout, dropout_cache_list[idx-2])

      dout = relu_backward(dout, relu_cache_list[idx-2])
      
      if self.use_batchnorm:
        dout, dgamma, dbeta = batchnorm_backward(dout, bn_cache_list[idx - 2])
